...
- 出站：AI 写 marker，如 `<send to="email:Zephyr">...</send>`，marker registry 解析后发布到 `fiam/dispatch/<target>`。
...
capabilities = ["receive_text", "dispatch_text", "dispatch_attachment"]
...
- `dispatch_targets` 是出站 marker 和 topic 叶子名，例如 `<send to="email:Zephyr">...</send>` 对应 `fiam/dispatch/email`。
- `dispatch_attachment` 表示 bridge 支持 ObjectStore-backed outbound attachments。dispatch payload 中只能引用 `attachments[*].object_hash`，bridge 负责 materialize bytes；不得把 path/base64/raw content 当作正式能力合同。
...
- `email`：长期保留，适合作为低频、耐久异步通道；支持 ObjectStore-backed SMTP MIME attachments。
...
- `events.sqlite3`：所有经历的事实层。
- `store/objects/`：附件、长结果、生成文件、截图等稳定对象。
- `Pool`：从事实层派生的长期事件图谱。
- `RecallContext`：每个 turn 的动态 recall 注入；外部 hook 只消费一次性 `pending_recall.md`。
...
- 去自制 app：走 Fiam API，入站发布为 `fiam/receive/app`，出站用 `<send to="app:Zephyr">...</send>`。服务器可以按任务路由到 Claude API 或唤醒 CC。
