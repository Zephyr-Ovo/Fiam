...
- Stroll top-right control expands/collapses the map inside the Stroll page so the map covers the full Stroll root while conversation/composer stay stable above it. Do not hide Android system bars globally or from Stroll route code; Home must keep its existing proportions and the escape/strollentry image must stay at its original visual size even if the pull hit target is larger.
