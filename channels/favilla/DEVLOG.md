...
- Android route entry correction: the Home Stroll handle now supports tap as well as pull because installed WebView/ADB swipes did not reliably trigger the pointer pull path. Stroll can request fullscreen/status-bar hide while mounted and restores on route exit; this must remain route-scoped and never live in `MainActivity`.
