...
- `[external]` 格式：`[chat/favilla:Zephyr] 消息内容` 或 `[email:Mom] 邮件内容`；Favilla 还会出现 `[chat/favilla:Zephyr] [标记] todo`（marker grid 快捷按钮）和 `[chat/favilla:Zephyr] [图像] <描述>`（拍照/选图）。
...
| 方式 | 触发 | 消息格式 |
|------|------|----------|
| 唤醒 | daemon 用 `claude -p` | 外部消息以 `[channel/surface:from_name] text` 送达（例 `[chat/favilla:Zephyr] hi`） |
