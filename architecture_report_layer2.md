...
daemon 在启动时用 `bus.subscribe(RECEIVE_ALL, _on_receive)` 订阅了 topic 模式 `fiam/receive/#`。当 broker 将消息推送到该 topic 时，paho 的网络线程回调 `_on_receive(source, payload)`；nested receive topic 会把 `fiam/receive/` 之后的完整 suffix 作为 source 传入。
...
**步骤 d — 漂移检测（auto 模式）**  
若 `_last_vec` 不为 None，且当前 beat 的 source 不是 `"action"`，调用 `detect_drift(last_vec, current_vec, threshold=0.65)`。`detect_drift` 计算两个向量的余弦相似度，若低于 0.65 则触发 `on_drift(current_vec)` 回调。该回调在 daemon 中被绑定为 pending recall callback（见第三节）。`_last_vec` 随后更新为当前向量。
...
当 `detect_drift` 触发时，daemon 的 pending recall callback 会接收 `query_vec`（在 Conductor 处理 beat 的线程内，即 daemon 主线程）。
...
**步骤 3 — 采样与生成上下文**（`retrieve` + `build_recall_context`）：将最终激活值作为 Bernoulli 概率，对每个节点独立采样，选出激活值大于 0.4 的 eligible Pool nodes，最多保留 `top_k=3` 个。

对每个命中的 event，调用 `pool.read_body(event_id)` 读取 `.md` 文件内容，截取前 200 字符（超出则加 `...`），计算时间距今的语言描述（"刚才"/"X 小时前"/"X 天前"/"X 个月前"）。

结果生成 bounded `RecallContext`，每条包含 event id、时间 hint、activation、短 summary、object refs。内部 app/API prompt 直接传这个对象；外部 CC hook 如需文件投递，只写一次性 `pending_recall.md`，注入后删除。
...
**Section 2 — recall**：仅当 `pending_recall.md` 非空时才注入。先原子性重命名为 `.processing`，读取后组成 `[recall]\n<内容>` 块，最后删除 processing 文件；没有 dirty marker fallback。
...
| `pending_recall.md` | inject.sh | pending recall callback | 外部 hook 的一次性联想片段 |
