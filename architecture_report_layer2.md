...
- 调用 `extract_sleep_tag` 提取最后一个 `<<SLEEP:until:reason>>` 标记。若存在，将 `state=sleep`、`until` 和 `reason` 写入 `self/ai_state.json`，同时将 active session 归档到 `self/retired/<ts>_sleep.json` 并清空 `active_session.json`。旧 `self/sleep_state.json` 会在读取时自动迁移。
...
对每条 due 条目，先检查 budget，若超预算则调用 `mark_fired(entry, success=False)` 不实际唤醒。否则检查 `ai_state`；若 sleep 为 `"open"`，先清除 sleep 状态再唤醒；若 sleep 为明确时间点且当前时间未过，调用 `mark_fired(entry, success=True)` 跳过（AI 的主动休眠优先于日程唤醒）。
