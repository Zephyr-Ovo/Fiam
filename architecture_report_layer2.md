...
### 1.1 bridge_tg.py — 拉取 → 发布

`bridge_tg.py` 以独立进程运行，每隔 `config.tg_poll_interval`（最小值 10 秒）调用一次 `fetch_tg_inbox(config)`。

`fetch_tg_inbox` 向 `https://api.telegram.org/bot{token}/getUpdates` 发起 HTTP POST，携带 `offset`（上次处理的 update_id + 1）和 `timeout=5`。响应 JSON 的 `result` 字段是一个 update 列表。对每条 update，从 `message.text` 提取文本，从 `message.from.first_name` 提取发送者名称，从 `message.date`（Unix 时间戳）转换为 `datetime`。这些字段被组装成字典 `{"text", "from_name", "t", ...}`，作为本轮拉取结果返回给 bridge。

bridge 对返回的每条消息，先检查 `plugins/tg/plugin.toml` 是否仍启用；若启用，调用 `bus.publish_receive("tg", {...})`。

`publish_receive` 在 `bus.py` 中的行为：向 topic `fiam/receive/tg` 发布一条 MQTT 消息，payload 为 JSON 序列化后的字典，包含 `text`、`from_name`、`source`、`t`（ISO 字符串）。`datetime` 对象在此处被序列化为 `isoformat()` 字符串。QoS = 1。
...
**步骤 b — 嵌入与冻结特征**  
调用 `embedder.embed(beat.text)`，将纯内容文本转化为 bge-m3 float32 向量。随后 `FeatureStore.append_beat_vector()` 将向量幂等写入 `store/features/flow_vectors.npy`，并在 `flow_index.jsonl` 记录 beat hash、时间、source、text_hash、model_id、dim。若 embed 调用抛出异常，beat 已写入 flow.jsonl 但不会产生冻结向量，也不继续走后续流程。
...
- 用 `_OUTBOUND_RE` 正则（匹配 `[→tg:Name]` 和 `[→email:Name]`）提取出站消息标记，每个匹配得到 `(channel, recipient, body)` 三元组，调用 `conductor.dispatch(channel, recipient, body)`（走 MQTT bus 发布到 `fiam/dispatch/<target>`）。

- 调用 `extract_wake_tags` 提取 `<<WAKE:ISO:type:reason>>` 格式的自唤醒标记，批量追加到 `schedule.jsonl`。

- 调用 `extract_sleep_tag` 提取最后一个 `<<SLEEP:until:reason>>` 标记。若存在，将 `sleeping_until` 和 `reason` 写入 `self/sleep_state.json`，同时将 active session 归档到 `self/retired/<ts>_sleep.json` 并清空 `active_session.json`。
...
**来源 A — _wake_session 内直接 dispatch**：CC 回复中的 `[→tg:Name]` 标记，由 `_extract_and_dispatch` 解析后调用 `conductor.dispatch(channel, recipient, body)`，发布到 `fiam/dispatch/<target>` MQTT topic。

**来源 B — outbox.sh hook + sweep_outbox**：CC 的 `Stop` hook 运行 `outbox.sh`，从 stdin 读取 CC 传入的 JSON（含 `stopHookData.transcript`），提取最后一条 assistant 消息文本，用 Python 正则找出 `[→channel:recipient]` 块，每个匹配写入 `outbox/<timestamp>.md` 文件，frontmatter 含 `to`、`via`（telegram/email）、`priority`。  
...
**bridge 侧接收 dispatch**：`bridge_tg.py` 订阅 `fiam/dispatch/tg`，回调 `_on_dispatch` 接收 payload，取 `text` 和 `recipient`。调用 `_extract_stickers(text, config)` 从文本中找出 `[sticker:关键词]` 标记，从 `config.sticker_index_path`（`stickers/index.json`）按关键词查找对应的 TG sticker file_id，将带 sticker 标记的部分从文本中替换移除。正文部分调用 `_tg_send_segmented`（分段发送 + 模拟 typing 指示器），每个 sticker 调用 `_tg_send_sticker`。

---

## 九、调度器路径（scheduler）

daemon 主循环每轮调用 `archive_stale(config)`，清理 `schedule.jsonl` 中已过期（超过 grace 窗口）或超过最大尝试次数的条目，移入 `schedule_archive.jsonl`。

随后调用 `load_due(config)`，返回所有 `wake_at` ≤ 当前 UTC 时间且状态为 pending 的条目。

对每条 due 条目，先检查 budget，若超预算则调用 `mark_fired(entry, success=False)` 不实际唤醒。否则检查 sleep_state；若 sleep 为 `"open"`，先清除 sleep_state 再唤醒；若 sleep 为明确时间点且当前时间未过，调用 `mark_fired(entry, success=True)` 跳过（AI 的主动休眠优先于日程唤醒）。

通过 budget 和 sleep 检查后，调用 `_wake_session(config, "[scheduled:<type>] <reason>", tag="sched", conductor=_conductor)`，流程与第六节完全相同。
...
| `features/flow_vectors.npy` | annotator, 训练脚本 | FeatureStore | 冻结 beat 向量矩阵 |
| `features/flow_index.jsonl` | annotator, 训练脚本 | FeatureStore | beat hash 到向量行号的索引 |
...
| `self/sleep_state.json` | daemon | _save_sleep_state | sleeping_until + reason |
| `self/schedule.jsonl` | scheduler | append_to_schedule | WAKE 任务队列 |
| `self/comm_state.json` | daemon | AI 直接写 | notify / mute / block |
